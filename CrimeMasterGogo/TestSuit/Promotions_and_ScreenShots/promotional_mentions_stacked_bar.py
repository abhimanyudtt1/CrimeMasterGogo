...
class TestLoginToUI_1(unittest.TestCase):
    def test_testLogin(self):
        testResults.summary("Testing Promotions : Mentions Stacked Bars")
        driver = gogo.appUI.init()
        gogo.appUI.login()
        gogo.appUI.goToInsight('promotional_mentions_stacked_bars')
        gogo.appUI.selectDate()
        gogo.appUI.selectFilter('merchant', 'amazon')
        #gogo.appUI.waitTillLoading()
        print "Checking if URL is correct"
        expected_url = '%s/insights?insight=promoYoy&entity=promo&date=1438560000000,1441584000000&merchant=6,2147483647,2147483646,2147483644,2147483645' % parser.URL
