...
class TestLoginToUI_1(unittest.TestCase):
    def test_testLogin(self):
        testResults.summary("Testing Brand - Discount Level")
        driver = gogo.appUI.init()
        gogo.appUI.login()
        gogo.appUI.goToInsight('brand_discount_level')
        ...
        expected_url = '%s/insights?insight=productBICC_discountBands&entity=sku&date=1438560000000,1441584000000&category=1&brand=Coffeemaker&mode=sku' % parser.URL
