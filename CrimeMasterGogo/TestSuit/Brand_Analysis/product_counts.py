...
class TestLoginToUI_1(unittest.TestCase):
    def test_testLogin(self):
        testResults.summary("Testing Brand - Product Count")
        driver = gogo.appUI.init()
        gogo.appUI.login()
        gogo.appUI.goToInsight('brand_product_counts')
        ...
        expected_url = '%s/insights?insight=productBICC_skuCount&entity=sku&date=1438560000000,1441584000000&category=1&brand=Coffeemaker&mode=sku' % parser.URL
