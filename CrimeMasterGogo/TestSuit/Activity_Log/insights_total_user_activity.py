...
class TestLoginToUI_1(unittest.TestCase):
    def test_testLogin(self):
        testResults.summary("Testing Activity : Insights Total User Activity")
        driver = gogo.appUI.init()
        gogo.appUI.login()
        gogo.appUI.goToInsight('insights_total_user_activity')
        gogo.appUI.selectDate()
        gogo.appUI.waitTillLoading()
        print "Checking if URL is correct"
        expected_url = '%s/insights?insight=activityInsightCountUser&entity=activity&date=1438560000000,1441584000000' % parser.URL
