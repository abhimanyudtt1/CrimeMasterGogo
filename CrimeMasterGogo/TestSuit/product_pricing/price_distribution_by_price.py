...
class TestLoginToUI_1(unittest.TestCase):
    def test_testLogin(self):
        testResults.summary("Testing Product pricing - Product Distribution by Price")
        driver = gogo.appUI.init()
        gogo.appUI.login()
        gogo.appUI.goToInsight('avg_price_product_distribution_by_price')
        ...
        expected_url = '%s/insights?insight=priceGraph&entity=sku&date=1438560000000&category=1&mode=sku' % parser.URL
