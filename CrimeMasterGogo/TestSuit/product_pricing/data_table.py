...
class TestLoginToUI_1(unittest.TestCase):
    def test_testLogin(self):
        testResults.summary("Testing Product pricing - data_table")
        driver = gogo.appUI.init()
        gogo.appUI.login()
        gogo.appUI.goToInsight('avg_price_data_table')
        ...
        expected_url = '%s/insights?insight=priceTable&entity=sku&date=1438560000000&category=1&mode=sku' % parser.URL
