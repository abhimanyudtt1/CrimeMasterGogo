...
class TestLoginToUI_1(unittest.TestCase):
    def test_testLogin(self):
        testResults.summary("Testing Product pricing - Relative Bar Chart")
        driver = gogo.appUI.init()
        gogo.appUI.login()
        gogo.appUI.goToInsight('avg_price_relative_bar_chart')
        ...
        expected_url = '%s/insights?insight=priceBar&entity=sku&date=1438560000000&category=1&mode=sku' % parser.URL
